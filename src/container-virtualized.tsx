...
export default class StickyContainerVirtualized extends React.Component<
    Props,
    State
> {
    public refs: {
    ...
    public render() {
        ...
        return (
            <div ref="container" style={style} className={this.props.className}>
                <div
    ...
    public refresh() {
        this.stickyHeaderHandler(
            ReactDOM.findDOMNode(this.refs.list).scrollTop,
        );
    }

    private scrollHandler = (params: {
        clientHeight: number;
        scrollHeight: number;
        scrollTop: number;
    }) => {
    ...
    public currentStickyElementIdentifier() {
        ...
        const currentStickyElement = list[
            this.state.index
        ] as React.ReactElement<StickyProps>;
    ...
    private stickyHeaderHandler = (scrollTop: number) => {
        ...
        const list = React.Children.toArray(this.props.children);
        const stickyHeight =
            this.props.stickyElementHeight || this.props.elementHeight;
        while (scrollTop > scroll) {
            child = list[idx] as React.ReactElement<any>;
            scroll +=
                child.type === StickyElement
                    ? stickyHeight
        ...
        if (index !== this.state.index || top !== this.state.top) {
            const width =
                ReactDOM.findDOMNode(this.refs.list).clientWidth + "px";
            this.setState({
                width,
                index,
                top,
    ...
    private getCover = () => {
        if (this.state.index === null || this.state.top !== 0) return null;
    ...
    private getSticky = () => {
        ...
        return (
            <div className={this.props.stickyClassName} style={style}>
    ...
    private getChildren = () => {
        ...
        const renderRow: ListRowRenderer = ({ index, key, style }) => {
            ...
            return React.cloneElement(
                child,
                Object.assign({}, child.props, { style: finalStyle }),
            );
        };
        const rowHeight = ({ index }: Index) => {
            const child = list[index] as React.ReactElement<any>;
            return child.type === StickyElement
                ? stickyHeight
                : this.props.elementHeight;
        };
